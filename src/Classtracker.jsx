...
export default function App() {
  ...
  const [editingScheduleWeek, setEditingScheduleWeek] = useState(null); // which week is being edited
  const [deleteScheduleWeek, setDeleteScheduleWeek] = useState(null);

  // Completed class days (stored locally per week-day)
  ...
  // Toggle completed for a schedule day in navbar
  // When completed → add a session to the API for that day's actual date
  // When uncompleted → delete that session
  const toggleDayCompleted = async (weekStart, day, scheduleInfo) => {
    const key = `${weekStart}__${day}`;
    const alreadyDone = !!completedDays[key];

    if (!alreadyDone) {
      // Mark complete → figure out the actual date of this day in the week
      const dayIdx = DAYS.indexOf(day); // 0=Mon ... 6=Sun
      const weekStartDate = new Date(weekStart + "T12:00:00");
      weekStartDate.setDate(weekStartDate.getDate() + dayIdx);
      const classDate = weekStartDate.toISOString().slice(0, 10);
      const classStart = scheduleInfo?.belgiumTime
        ? belgiumToPakistan(scheduleInfo.belgiumTime)
        : "";
      ...
      const body = {
        ...
        duration: null,
        notes: `Schedule class — ${DAYS_FULL[dayIdx]}`,
        paid: false,
      };
      try {
        ...
        // Store session id in completedDays so we can delete it later
        const newData = { ...completedDays, [key]: saved._id };
        saveCompletedDays(newData);
      } catch (e) {
        console.error("Failed to add session", e);
      }
    } else {
      // Un-mark → delete the session we added
  ...
  const isDayCompleted = (weekStart, day) => {
    return !!completedDays[`${weekStart}__${day}`];
  };
  ...
  const deleteSchedule = (weekKey) => {
    ...
    setScheduleData(newData);
    localStorage.setItem("class_schedules", JSON.stringify(newData));
    // also remove completed days for this week
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: (thisWeekSchedule && thisWeekSchedule.days?.length > 0) ? "12px" : "0" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            <button onClick={showForm ? () => setShowForm(false) : openForm} style={{
              ...
              fontFamily: "'Georgia', serif", fontSize: "13px", fontWeight: "bold", cursor: "pointer",
            }}>{showForm ? "✕ Cancel" : "+ Add Class"}</button>
          </div>
        </div>
